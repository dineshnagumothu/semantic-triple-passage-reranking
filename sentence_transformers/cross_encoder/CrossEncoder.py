...
#from transformers import BertModelForSequenceClassification, BertTokenizer, BertConfig
import numpy as np
import logging
import os
...
class CrossEncoder():
    def __init__(self, model_name:str, num_labels:int = None, max_length:int = None, device:str = None, tokenizer_args:Dict = {},
        ...
        #self.model = BertModelForSequenceClassification.from_pretrained(model_name, config=self.config, **automodel_args)
        #self.tokenizer = BertTokenizer.from_pretrained(model_name, **tokenizer_args)
    ...
    def triple_loss(self,logits, feature_overlap_scores):
        ...
        print ("Triple loss:"+ str(new_loss))
        return new_loss
    ...
    def fit(self,
        ...
        if loss_fct is None:
            loss_fct = nn.BCEWithLogitsLoss(reduction="mean") if self.config.num_labels == 1 else nn.CrossEntropyLoss()

        #activation_fct = self.default_activation_function
        #activation_fct = nn.Softmax()
        
        #print model
        #summary(self.model, input_size=(16,512))
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        ...
        if evaluator is not None:
            ...
            if score > self.best_score:
                ...
                self.patience_steps = 0
                if save_best_model:
                    output_path=output_path+"_best_model"
