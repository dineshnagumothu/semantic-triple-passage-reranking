...
class CERerankingEvaluator:
    ...
    """
    def __init__(self, samples, mrr_at_k: int = 10, name: str = '', write_csv: bool = True, num_labels=1):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for instance in self.samples:
            ...
            pos_overlap_scores = list(instance['pos_coverage_scores'])
            neg_overlap_scores = list(instance['neg_coverage_scores'])
            docs = positive + negative
            overlap_scores = pos_overlap_scores + neg_overlap_scores
            ...
            for i in range(len(docs)):
                model_input.append({"sentences":[query, docs[i]], "overlap_scores":overlap_scores[i]})
            #print (model_input)
            ...
            if(self.num_labels > 1):
                ...
                for pred_score in pred_scores:
                    pred_scores_new.append(pred_score[1])
                pred_scores = np.array(pred_scores_new)
                #print (pred_scores)
            '''Softmax part end'''
